...
class DQN_Agent:
    ...
    def learn(self, batch_size, done):
        ...
        with torch.no_grad():  
            q_return = self.target_network(next_states)          
            next_target_q_value = q_return.max(dim=1, keepdim=True)[0] # not argmax (cause we want the maxmimum q-value, not the action that maximize it)
        q_return = torch.mean(q_return, dim=1, keepdim=True)[0]
    ...
    def soft_update(self, beta):
        ...
        for target_param, main_param in zip(self.target_network.parameters(), self.main_network.parameters()):
            target_param.data.copy_((1-beta) * main_param.data + beta * target_param.data)
        ...
        for episode in range(1, self.max_episodes+1):
            ...
            while not done and not truncation:
                ...
                if len(self.agent.replay_memory) > self.batch_size:
                    if first_state == 0:
                        first_state = 1
                        q_target = self.agent.learn(self.batch_size, (done or truncation)).item()
